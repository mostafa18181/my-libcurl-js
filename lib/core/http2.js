/**
 * Description:
 * This file implements the `Http2Client` class, which provides a flexible and efficient way to make HTTP/2 requests
 * with support for SSL/TLS configurations, custom headers, cookies, error handling, retries, and HTTP/2 server push.
 * It facilitates making requests like GET, POST, PUT, and DELETE, and handles complex scenarios such as stream errors
 * and connection management.
 *
 * Purpose of the File:
 * - Establish and manage HTTP/2 connections with optional SSL/TLS settings.
 * - Handle headers, cookies, and advanced stream management.
 * - Manage retries and handle various stream errors, including recoverable and fatal errors.
 * - Provide a simplified interface for making HTTP/2 requests with built-in stream handling.
 * - Support HTTP/2 server push for handling additional unsolicited responses.
 */
