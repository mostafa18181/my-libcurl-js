/**
 * Description:
 * This file implements an `HttpsClient` class that provides a robust and configurable HTTPS client
 * capable of handling SSL/TLS connections, cookies, retries, redirects, and custom headers. The client
 * can perform GET, POST, PUT, and DELETE requests with built-in error handling, response management,
 * and concurrent request queuing.
 *
 * Purpose of the File:
 * - Establish secure HTTPS connections with optional SSL/TLS configurations.
 * - Manage HTTP headers and cookies automatically.
 * - Handle redirects, retries, and timeouts for requests.
 * - Provide a simplified API for making HTTP requests (GET, POST, PUT, DELETE).
 * - Manage concurrent requests with a queue system.
 */
...
class HttpsClient {
    ...
    _buildOptions(method, path, additionalOptions = {}) {
        ...
        const options = {
            ...
            ...this.sslOptions, // Apply SSL/TLS settings
        };
