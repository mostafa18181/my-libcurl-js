/**
 * Description:
 * The `FtpClient` class provides a simple interface for interacting with FTP servers. It allows for connecting to FTP servers, uploading and downloading files, listing directories, creating and deleting files and directories, and renaming files. The class also supports retrying operations if they fail initially, providing resilience in case of temporary issues.
 *
 * Purpose of the File:
 * - Establish a connection with an FTP server.
 * - Perform file and directory operations such as upload, download, list, create, delete, and rename.
 * - Implement retry logic for resilience against transient errors.
 * - Provide methods to load FTPS options for secure connections using certificates.
 */
