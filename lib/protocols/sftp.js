/**
 * Description:
 * The `SftpClient` class provides an interface for interacting with remote servers using SFTP (Secure File Transfer Protocol). It enables uploading and downloading files and directories, managing remote directories, and performing file operations like renaming and deleting. The class supports automatic reconnection attempts and handles idle timeouts to manage resources efficiently.
 *
 * Purpose of the File:
 * - Establish and manage SFTP connections to remote servers.
 * - Perform secure file transfers (upload and download) between local and remote systems.
 * - Manage remote directories and their contents.
 * - Retry failed operations to improve resilience against transient errors.
 * - Handle secure connections using private key files for authentication.
 */
